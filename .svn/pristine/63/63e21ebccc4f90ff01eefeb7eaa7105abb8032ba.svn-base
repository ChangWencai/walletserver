...
func (self *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			adminObj, err := models.GetAdminById(userId)
			if err == nil && password == libs.Md5([]byte(self.getClientIp()+"|"+adminObj.Password+adminObj.Salt)) {
				self.userId = adminObj.Id
				self.loginName = adminObj.LoginName
				self.userName = adminObj.RealName
				self.user = adminObj
				self.AdminAuth()
			}

			isHasAuth := strings.Contains(self.allowUrl, self.controllerName+"/"+self.actionName)
			noAuth := "ajaxsave/ajaxdel/table/loginin/loginout/getnodes/start/show/ajaxapisave/index/group/public/env/code/apidetail/ajaxmodify"
			isNoAuth := strings.Contains(noAuth, self.actionName)
